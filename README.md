...
<p align="center">
 일렉트론 실행 <br/> GitHub Repo - <a href="https://github.com/parkyooni/LayerTune" target="_blank">Clint<br/>
...
- [핵심기능 구현 과정](#%ED%95%B5%EC%8B%AC%EA%B8%B0%EB%8A%A5-%EA%B5%AC%ED%98%84-%EA%B3%BC%EC%A0%95)
  ...
  - [[대시보드 - 프로젝트 구조] 계층 구조를 N-ary Tree 구조 알고리즘으로 보여줍니다.](#%EB%8C%80%EC%8B%9C%EB%B3%B4%EB%93%9C---%ED%94%84%EB%A1%9C%EC%A0%9D%ED%8A%B8-%EA%B5%AC%EC%A1%B0-%EA%B3%84%EC%B8%B5-%EA%B5%AC%EC%A1%B0%EB%A5%BC-n-ary-tree-%EA%B5%AC%EC%A1%B0-%EC%95%8C%EA%B3%A0%EB%A6%AC%EC%A6%98%EC%9C%BC%EB%A1%9C-%EB%B3%B4%EC%97%AC%EC%A4%8D%EB%8B%88%EB%8B%A4)
...
### [프로젝트 생성 - 파일 경로 선택] 클라이언트에서만 파일 경로를 보여줄 수 있습니다.
...
### [대시보드 - 프로젝트 구조] 계층 구조를 N-ary Tree 구조 알고리즘으로 보여줍니다.

> 생성된 프로젝트의 폴더 내부 파일 및 폴더의 계층을 재귀적으로 반복하여 마지막 파일까지 표시합니다. 파일의 경우, 확장자별로 아이콘을 별도로 표시하여 시각적으로 가독성을 높이도록 의도하였습니다.

#### 구체화 <br/>

HTML 태그에서 `<ul>`과 `<li>`를 사용하여 영역을 잡은 후, 재귀적으로 함수를 작업하여 `<ul>/<li>/<ul>/<li>…`의 계층 구조로 접근합니다.

#### 방안 적용 및 결과 <br/>

퍼블리싱 단계에서 Node.js의 파일 시스템 메소드를 사용하여 계층 구조에 접근하려고 했으나, 읽어들이는 데이터가 계층 형태가 아닌 평문(즉, 단순한 문자열)으로 가져오는 문제가 발생했습니다.이 문제로 인해 파일과 폴더의 계층적 구조를 제대로 표현할 수 없었습니다.

이러한 문제를 해결하기 위해, 파일 시스템의 디렉터리 구조를 표현하는 데 일반적으로 사용되는 `N-ary Tree 구조`를 적용하기로 결정했습니다.

- `N-ary Tree`란 각 노드가 여러 개의 자식 노드를 가질 수 있는 트리 구조입니다.

  - 루트 노드 설정 : 현재 작업 중인 폴더를 트리의 루트(root)로 시작점으로 설정합니다.
  - 자식 노드 추가 : 배열을 사용하여 루트 아래에 여러 개의 자식 노드를 추가합니다. <br/>
    각 폴더는 여러 자식 파일이나 폴더를 가질 수 있으므로, N-ary Tree의 장점을 활용하여 유연하게 구조를 구성합니다.
  - 재귀적 탐색 : 루트에서부터 폴더를 만나면, 그 폴더 내부의 자식 요소(파일 및 하위 폴더)를 가져옵니다.<br/>
    더 이상 자식 요소가 없는 경우에는 마지막 파일까지 읽어들여 전체 폴더 구조를 완성합니다.
...
|                                퍼블리싱 단계의 계층 형태 프로젝트 구조                                |                     파일 시스템에 N-ary Tree 적용한 계층 형태 프로젝트 구조                     |

#### 개선점 <br/>

코드의 가독성을 높이기 위해, 확장자별 파일 아이콘을 별도의 함수로 정리할 계획입니다. 또한, main.js의 프로세스 로직을 유틸리티 함수로 분리하여 유지보수 측면을 고려할 예정입니다.
